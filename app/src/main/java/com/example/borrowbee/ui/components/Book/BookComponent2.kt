...
import com.example.borrowbee.data.entities.toBookModel
import com.example.borrowbee.ui.tabs.openBookDetailsActivity
import com.example.borrowbee.ui.theme.robotoCondenseFamily

@Composable
fun BookComponent2(bookEntity: BookEntity, context: Context) {
    Column(
        modifier = Modifier
            .wrapContentHeight()
            .padding(8.dp)
            .clickable { openBookDetailsActivity(context, bookEntity.toBookModel()) },
...
@Composable
fun MyBookItem(bookEntity: BookEntity, context: Context) {
    Column(
        modifier = Modifier
            .wrapContentHeight()
            .width(140.dp)
        ...
        Row(
            ...
            LinearProgressIndicator(
                //progress = bookEntity.bookProgress,
                modifier = Modifier.width(5.dp),
                color = colorResource(id = R.color.progress_color),
                trackColor = MaterialTheme.colorScheme.onBackground,
            )
            Text(
                "${(bookEntity.bookProgress * 5).toInt()}%",
                color = MaterialTheme.colorScheme.onBackground,
                fontSize = 13.sp,
                fontFamily = robotoCondenseFamily,
